...
latentode:
    IndependentLatentODE. Each bus is encoded and evolved independently;
    physical edges are never used.

lgode:
    Original LG-ODE GTrans recognition encoder, linear decoder, and graph ODE
    with fixed unit weights on existing physical edges.

atode:
    The same GTrans recognition encoder, decoder, latent dimensions,
    augmentation, graph ODE architecture, solver, tolerances, dropout,
    likelihood, and prior as LG-ODE. The only generative-model difference is
    the time-dependent edge-weight provider.

Expected interfaces from the controlled repository modifications
----------------------------------------------------------------
The modified ``lib.diffeq_solver.DiffeqSolver`` is expected to accept:
...
The ``lib.attention_transport.AttentionTransport`` provider is expected to be
a differentiable nn.Module that can be queried repeatedly without mutation.

This module does not contain transport logic, dataset logic, training loops,
or result writing.
"""
...
class PowerGridModelConfig:
    ...
    def common_graph_model_signature(self) -> Dict[str, Any]:
        """
        Return settings that must be identical for LG-ODE and AT-ODE.
        """
...
def _has_argument(args: Any, name: str) -> bool:
...
def _read_argument(
    ...
    *names: str,
    ...
    """
    Read the first available argument name from a namespace or mapping.

    Aliases cover both the new runner names and the original repository names.
    """
...
def _resolve_config(
    ...
    solver = str(
        _read_argument(args, "solver", default="dopri5")
    )
    rtol = float(
        _read_argument(args, "rtol", default=1e-3)
    )
    atol = float(
        _read_argument(args, "atol", default=1e-4)
    )
    dropout = float(
        _read_argument(args, "dropout", default=0.2)
    )
    ...
    if encoder_type.lower() != "gtrans":
        raise ValueError(
            "The power-grid LG-ODE protocol requires the original GTrans "
            f"encoder; got {encoder_type!r}"
        )

    if ode_type.lower() != "nri":
        raise ValueError(
            "The power-grid LG-ODE protocol requires the NRI graph ODE; "
            f"got {ode_type!r}"
        )
...
def _solver_args_namespace(
    ...
    Original LG-ODE components read several settings directly from ``args``.
    """
...
def _deterministic_initialization(
    ...
    """
    Initialize common LG-ODE and AT-ODE components identically.

    The RNG state outside the factory is restored on exit. AT-ODE's additional
    transport module is initialized only after all shared modules.
    """
...
def _standard_normal_prior(
    ...
    mean = torch.zeros(1, latent_dim, device=device)
    std = torch.ones(1, latent_dim, device=device)
...
def _construct_with_supported_keywords(
    ...
    This keeps the factory compatible with controlled solver/provider revisions
    that add optional keyword arguments without changing existing positional
    arguments.
    """
...
def _make_decoder(config: PowerGridModelConfig) -> Decoder:
    return Decoder(
        latent_dim=config.latent_dim,
...
def _make_graph_solver(
    ...
    *,
...
) -> Tuple[GraphODEFunc, DiffeqSolver]:
    ...
    # Controlled solver revisions may expose an explicit setter instead of
    ...
    # Standardized, auditable metadata even if the solver stores equivalent
    # values under older attribute names.
...
def _verify_solver_protocol(
    ...
    expected_mode = "fixed" if model_name == "lgode" else "transport"
...
def _attach_model_metadata(
    ...
    """
    Attach non-parameter protocol metadata for tests and result writing.
    """
    ...
    # Registering this on every model would incorrectly suggest that the
    # graph-free baseline consumes physical edges. Store it only on graph
    # models.
...
def _build_graph_latent_ode(
    ...
    solver_args = _solver_args_namespace(args, config)

    # These three modules are shared architecturally and initialized before
    # AT-ODE's additional transport parameters. Rebuilding LG-ODE and AT-ODE
    # with the same seed therefore gives identical shared initial parameters.
    encoder = _make_gtrans_encoder(config).to(device)
    decoder = _make_decoder(config).to(device)

    if config.model_name == "lgode":
        edge_weight_mode = "fixed"
        edge_weight_provider = None
    else:
        edge_weight_mode = "transport"
    ...
    ode_function, solver = _make_graph_solver(
        config,
        solver_args,
        edge_index,
        device,
...
def build_powergrid_lgode_model(
    ...
    """
    Build one of the four power-grid experiment models.
    ...
    args:
        argparse namespace or mapping containing model hyperparameters.
    device:
        PyTorch device.
    ...
    Notes
    -----
    LG-ODE and AT-ODE are initialized under the same deterministic RNG scope.
    Given the same seed, all shared encoder, decoder, and ODE parameters start
    identically. AT-ODE's additional transport parameters are initialized only
...
def shared_graph_state_dict(
    ...
    """
    Extract parameters that LG-ODE and AT-ODE are required to share.

    Transport-provider parameters are excluded.
    """
...
def assert_lgode_atode_protocol_match(
    ...
    *,
    ...
    This helper is intended for protocol tests and runner startup validation.
    """

    lgode_config = getattr(lgode, "powergrid_config", None)
    atode_config = getattr(atode, "powergrid_config", None)

    if not isinstance(lgode_config, PowerGridModelConfig):
        raise TypeError(
            "lgode was not built by build_powergrid_lgode_model"
        )
    if not isinstance(atode_config, PowerGridModelConfig):
        raise TypeError(
    ...
    if lgode_config.model_name != "lgode":
        raise ValueError(
            f"First model must be LG-ODE; got {lgode_config.model_name}"
        )
    if atode_config.model_name != "atode":
        raise ValueError(
            f"Second model must be AT-ODE; got {atode_config.model_name}"
        )

    lgode_signature = lgode_config.common_graph_model_signature()
    atode_signature = atode_config.common_graph_model_signature()

    if lgode_signature != atode_signature:
        differing = {
            key: (lgode_signature[key], atode_signature[key])
            for key in lgode_signature
            if lgode_signature[key] != atode_signature[key]
        }
    ...
    if getattr(lgode, "edge_weight_mode", None) != "fixed":
        raise AssertionError("LG-ODE must use fixed edge weights")
    if getattr(atode, "edge_weight_mode", None) != "transport":
        raise AssertionError("AT-ODE must use transport edge weights")
    ...
    common_names = sorted(
        set(lgode_state).intersection(atode_state)
    )
